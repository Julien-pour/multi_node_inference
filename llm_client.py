...
class LLMClient:
    def __init__(self, model: str, cfg_generation: dict, base_url: str ="",
                  api_key: str="None", online: bool = True, gpu=1,
        ...
        # qwen3 detection for specific stuff link to qwen3 (yarn, hybrid thinking, etc.) 
        # some qwen 3 are no more hybrid so need to rework this
        self.qwen3 = "qwen3" in model_lower 
        self.seed = seed
        self.fp8 = fp8
         # fp8 is in model name, set fp8 to False as it will auto load in fp8
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="LLM Client Launcher")
    parser.add_argument("--model", type=str, required=True, default= "/lustre/fsn1/projects/rech/imi/uqv82bm/hf/Qwen3-Coder-480B-A35B-Instruct-FP8", help="Path to the model")
    parser.add_argument("--gpu-memory", type=float, default=0.85, help="GPU memory utilization fraction")
    parser.add_argument("--log-level", type=str, default="info", help="Log level for the server (debug, info, warning, error, critical)")
    parser.add_argument("--max-model-length", type=int, default=20000, help="Maximum model length for the server")
    ...
    model = args.model
    # test multi nodes
    # model = "/lustre/fsn1/projects/rech/imi/uqv82bm/hf/Qwen3-Coder-480B-A35B-Instruct-FP8"#Qwen2.5-14B-Instruct" #DeepSeek-R1-0528"
    cfg_generation = {"model": model, "temperature": 0.6}

    # compute TP for SGLang
    n_nodes = int(os.environ.get('SLURM_NNODES', 1))
    gpus_per_node = int(os.environ.get('SLURM_GPUS_ON_NODE', 4))
    gpu = n_nodes * gpus_per_node
    print(f"SLURM_NNODES: {n_nodes}, SLURM_GPUS_ON_NODE: {gpus_per_node}, Total GPUs (tp): {gpu}")

    local_server = True
    llm = LLMClient(model=model, cfg_generation=cfg_generation,
                     gpu=gpu, local_server=local_server,
                     fp8=args.fp8, sglang=True, ep_moe=args.ep_moe,
                     gpu_memory=args.gpu_memory, log_level=args.log_level,
